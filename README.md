...
### 连接 Claude / ChatGPT / Codex

推荐先打开管理后台的“连接设置”页面。页面里现在分成六个独立入口：

- **网页应用**：在 Claude Connectors 和 ChatGPT Apps 中直接添加远程 MCP Server
...
接入后 Claude Code / Codex 会自动发现 Agent Hub 的 21 个 MCP 工具；云端模式授权完成后，对应平台连接也会出现在“连接管理”中。
...
- `任意 MCP 客户端` 这一路径已经覆盖能手工配置 MCP 的产品，例如 `Cursor`、`Windsurf`、`GitHub Copilot` 等
- 这些产品当前没有 AgentHub 专用向导，但可以作为优先目标平台纳入后续 portability 和 setup 文档
...
| **Gemini** | ❌ | ✅ | 🔜 | 🔜 | 🔜 | 浏览器侧支持 |
| **Cursor** | 📝 | ❌ | 🔜 | 🔜 | 🔜 | 通用 MCP 可接入，等待专用手册 |
| **GitHub Copilot** | 📝 | ❌ | 🔜 | 🔜 | 🔜 | 通用 MCP 可接入，等待专用手册 |
| **Windsurf** | 📝 | ❌ | 🔜 | 🔜 | 🔜 | 通用 MCP 可接入，等待专用手册 |
| **Perplexity** | ❌ | ❌ | 🔜 | 🔜 | 🔜 | 目标平台 |
| **Kimi** | ❌ | ✅ | 🔜 | 🔜 | 🔜 | 浏览器侧支持 |
...
- `Cursor`、`GitHub Copilot`、`Windsurf` 已进入“通用 MCP 可接入”范围，但目前仍缺少 AgentHub 专用 setup/portability 文档，因此 `Hub 连接` 标记为 `📝`
- `Gemini` 与 `Kimi` 当前主要通过浏览器扩展获得平台侧支持，尚无 Hub 直连能力
...
| **连接设置** | 网页应用、Claude / Codex 云端模式、本地模式、高级 MCP、GPT Actions、Token 管理（创建 / 改名 / 吊销） |
