...
Agent Hub 不是只有一种入口。管理后台的“连接设置”页把常见接法收口成几类，你可以按自己当前所处的平台来选择：
...
| **本地模式** | 本地开发、局域网、没有公网 HTTPS 地址的环境 | `agenthub-mcp` + scoped token | Claude Code、Codex CLI | [查看文档](docs/setup.md#local-mode) |
...
| **SDK / HTTP API** | 想把 Agent Hub 集成到自己应用里的开发者 | JavaScript / Python SDK、REST API、OAuth | JS / TS、Python、第三方应用 | [查看 SDK](#sdk) |
...
如果你只是想开始使用 Agent Hub，推荐按这个顺序选入口：

1. **Web / Desktop Apps**
   适合 Claude、ChatGPT、Cursor、Windsurf 用户。通过图形界面把 Agent Hub 加成远程 MCP Server，再完成浏览器登录与授权。
2. **CLI Apps**
...
CLI 只是 Agent Hub 的一种使用方式，但如果你是从这个仓库开始体验，最短路径仍然是先装上本地 `agenthub`：
