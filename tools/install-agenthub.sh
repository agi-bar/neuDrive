...
usage() {
  cat <<'EOF'
Usage: tools/install-agenthub.sh [--bin-dir DIR] [--force] [--no-rc]

Builds the local agenthub binary, installs it into a writable bin directory,
and adds that directory to your shell PATH if needed.
...
(
  cd "$REPO_ROOT"
  go build -o "${TMP_DIR}/${BIN_NAME}" ./cmd/agenthub
)
