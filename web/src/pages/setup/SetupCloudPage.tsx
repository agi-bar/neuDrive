...
export default function SetupCloudPage() {
  ...
  return (
    ...
    >
        ...
        {cloudPlatform === 'claude' ? (
          <>
            ...
            <p className="setup-note setup-note-first">
              {tx('把 neuDrive 添加到 Claude Code 的全局 MCP 配置中，然后在 Claude Code 里发起浏览器授权。', 'Add neuDrive to Claude Code global MCP config, then start browser authorization from Claude Code.')}
            </p>
            ...
            <ol className="setup-steps">
              <li>{tx('运行上面的命令后，neuDrive 会作为全局 MCP Server 出现在 Claude Code 中。', 'After running the command above, neuDrive appears in Claude Code as a global MCP server.')}</li>
              <li>{tx('打开 Claude Code，执行 ', 'Open Claude Code, run ')}<code>/mcp</code>{tx('，选择 ', ', choose ')}<code>neudrive</code>{tx('，然后开始认证。', ', then start authentication.')}</li>
            ...
            <p className="setup-note">
              {tx('授权完成后，你可以在 Claude Code 的 ', 'After authorization, you can reauthenticate or clear credentials from the Claude Code ')}<code>/mcp</code>{tx(' 菜单里重新认证或清除认证；neuDrive 侧也会在“连接管理”中显示这条平台连接。', ' menu. neuDrive will also show this platform connection under Connections.')}
            </p>
          </>
        ) : cloudPlatform === 'codex' ? (
          <>
            ...
            <p className="setup-note setup-note-first">
              {tx('把 neuDrive 添加到 Codex 的全局 MCP 配置中，然后用 Codex CLI 发起浏览器授权。', 'Add neuDrive to Codex global MCP config, then start browser authorization from Codex CLI.')}
            </p>
            ...
            <ol className="setup-steps">
              <li>{tx('运行 add 命令后，neuDrive 会写入 Codex 的用户级 MCP 配置，可在多个工作区复用。', 'After running the add command, neuDrive is written into Codex user-level MCP config and can be reused across workspaces.')}</li>
              <li>{tx('运行 ', 'Run ')}<code>codex mcp login neudrive</code>{tx(' 后，浏览器会打开授权页面。', ' and the browser will open the authorization page.')}</li>
            ...
            <p className="setup-note">
              {tx('授权完成后，neuDrive 侧会在“连接管理”中显示这条平台连接；需要重新认证时，可再次运行 ', 'After authorization, neuDrive will show this platform connection under Connections. To reauthenticate, run ')}<code>codex mcp login neudrive</code>{tx('。', '.')}
            </p>
          </>
          ...
          <>
            ...
            <p className="setup-note setup-note-first">
              {tx('把 neuDrive 添加到 Gemini 的远程 MCP 配置中，然后在 Gemini 里发起 OAuth 授权。', 'Add neuDrive to Gemini remote MCP config, then start OAuth authorization from Gemini.')}</p>
            ...
            <ol className="setup-steps">
              ...
              <li>{tx('添加完成后，打开 Gemini，执行 ', 'After adding it, open Gemini and run ')}<code>/mcp auth neudrive</code>{tx('；如果你用了别的 server 名称，把 ', '. If you used a different server name, replace ')}<code>neudrive</code>{tx(' 换成你自己的名称。', ' with your own name.')}</li>
              <li>{tx('浏览器会跳转到 neuDrive 的登录与授权页；完成登录和批准后，Gemini 会保存 OAuth 凭证。', 'The browser opens the neuDrive sign-in and authorization page. After login and approval, Gemini saves the OAuth credentials.')}</li>
              <li>{tx('接通后，你可以在 Gemini 里继续使用 ', 'After connecting, you can keep using ')}<code>/mcp</code>{tx(' 查看状态，或直接开始调用 neuDrive 的工具。', ' in Gemini to inspect status, or immediately start calling neuDrive tools.')}</li>
            </ol>
          ...
          <>
            ...
            <ol className="setup-steps">
              <li>{tx('先在项目目录的 ', 'First add the config above to ')}<code>.cursor/mcp.json</code>{tx('，或用户目录的 ', ' in the project directory, or ')}<code>~/.cursor/mcp.json</code>{tx(' 中加入上面的 ', ', then add the same ')}<code>neudrive</code>{tx(' 配置。', ' config in the user directory.')}</li>
              <li>{tx('运行 ', 'Run ')}<code>cursor-agent mcp login neudrive</code>{tx(' 后，Cursor Agent 会自动读取配置，并在浏览器中发起 neuDrive 的登录与授权。', '. Cursor Agent reads the config automatically and opens neuDrive sign-in and authorization in the browser.')}</li>
              <li>{tx('授权完成后，再运行 ', 'After authorization, run ')}<code>cursor-agent mcp list</code>{tx(' 检查状态；需要查看工具时，可以继续运行 ', ' to inspect status. To view tools, you can also run ')}<code>cursor-agent mcp list-tools neudrive</code>{tx('。', '.')}</li>
      ...
      <p className="setup-note">
        {tx('如果你本机已经有一个同名的本地 MCP 配置，例如旧的 ', 'If you already have a local MCP config with the same name, such as an old ')}<code>neudrive</code>{tx(' stdio 配置，建议先删除或改名，避免在平台列表中和远程 OAuth 连接混淆。', ' stdio config, delete it or rename it first so it does not get confused with the remote OAuth connection in platform lists.')}
      </p>
    </SetupSection>
  )
}
