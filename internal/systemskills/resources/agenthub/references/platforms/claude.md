...
- For one text/code skill whose files can be represented as strings, use `import_skill`. Nested paths like `scripts/run.py` are allowed.
...
- In that fallback flow, have Claude package a full zip, give the zip to the user for download, then direct the user to open the returned browser upload link and upload the zip manually.
