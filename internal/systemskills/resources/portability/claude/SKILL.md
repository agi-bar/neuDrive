...
- This is the closest Claude concept to durable neuDrive profile data.
...
- neuDrive does not have a first-class `style` entity, so preserve exact style text when it matters.
...
- Keep these subtypes separate when mapping into neuDrive.
...
- Use `write_file` for any imported file-like data that should be preserved in neuDrive, not just for projects. If some Claude material does not fit profile, memory, project, or skill cleanly, it may still be imported under a sensible custom directory structure.
...
- The agent may use `write_file` to patch one file, archive unsupported exports, or preserve additional imported Claude data under custom paths when no first-class neuDrive domain exists.
...
- If the probe POST fails in Claude Web, treat it as an outbound-domain restriction. Tell the user to open `Settings -> Capabilities -> Code execution and file creation -> Additional allowed domains` and add the neuDrive host, such as `neudrive.ai` on the official service. Then retry the probe and direct upload.
...
- All skill import paths land under `/skills/<name>/...` in neuDrive. Do not ask the user for a separate destination directory.
...
| Claude category | Claude source | neuDrive target | Preferred interface | Fallback interface | Current parity / notes |
| --- | --- | --- | --- | --- | --- |
...
8. Rebuild `project knowledge` into `/projects/<name>/...` when doing manual reconstruction. If using the official full export zip, note that the current importer writes project docs under `/skills/claude-<project>/...` rather than creating first-class neuDrive projects.
...
11. All skill import flows land under `/skills/<skill-name>/...` in neuDrive. A fallback upload page or upload command should target the `/skills` root by default and should not ask the user to pick another destination directory.
...
17. If the probe POST fails in Claude Web, tell the user to add the neuDrive host to `Settings -> Capabilities -> Code execution and file creation -> Additional allowed domains`, then retry the probe and direct upload.
...
19. When a Claude export contains useful files or metadata that do not fit a first-class neuDrive domain, preserve them with `write_file` under a clear custom directory structure instead of forcing them into a project.
...
When `/mnt/skills/user` exists and the goal is to move Claude Web skills into neuDrive, prefer a full archive:
...
4. First POST the returned `connectivity_probe_url`.
   ...
   - if the probe fails in Claude Web, tell the user to add the neuDrive host to `Settings -> Capabilities -> Code execution and file creation -> Additional allowed domains`, then retry
...
All of these paths still import into the neuDrive `/skills` root.
...
4. If the probe returns `200`, upload the zip directly from the Claude sandbox with the returned `curl_example`.
   - this is the preferred path when Claude can reach the neuDrive host directly
   - no user download or browser handoff is needed in this case
5. If the probe fails in Claude Web, explain that Claude likely blocked outbound POST to the neuDrive host.
6. Tell the user exactly how to unblock direct upload:
   - open `Settings -> Capabilities -> Code execution and file creation`
   - add the neuDrive host, such as `neudrive.ai`, under `Additional allowed domains`
...
10. Make it explicit that the browser upload page imports into the neuDrive `/skills` root by default. The user should not choose or type another destination path.
...
1. Inventory neuDrive data by Claude category, not just by file location:
...
9. Recreate `connectors / external sources` as manual setup instructions and selected-file manifests. Do not claim that neuDrive can restore third-party app connections natively.
...
- The current full Claude export importer does not map `users.json` into a first-class neuDrive domain.
...
- If the probe fails in Claude Web, the user may need to add the neuDrive host to `Settings -> Capabilities -> Code execution and file creation -> Additional allowed domains` before retrying direct upload.
