...
Hub 解法：在管理后台创建一个"协作"连接，绑定朋友的 Hub ID，限定可访问的范围（比如只能读 `memory/projects/industry-insights/` 下标记为 `education` 的内容）。朋友的 Agent 连入后，除了能读自己主人 Hub 里的全部内容，还能读到我共享出去的那一小块。两边的知识在朋友的 Agent 里汇合
...
│
├── identity/
│   ├── SKILL.md          ← 身份系统说明：支持哪些 auth provider、怎么绑定
│   ├── bindings/         ← 各平台的 OAuth 绑定
│   │   ├── github.json
│   │   ├── wechat.json
│   │   └── email.json
...
│
├── vault/
│   ├── SKILL.md          ← 加密存储说明：怎么申请 scope、调用协议
│   └── scopes/           ← 按领域分组的加密信息
│       ├── identity.personal    ← 身份证、护照（最高权限）
│       ├── auth.feishu          ← 飞书 token
│       ├── auth.openai          ← OpenAI API key
│       └── finance.bank         ← 银行信息
│
├── skills/               ← 用户积累的能力库（可直接复用现有 .skill 文件）
│   ├── cyberzen-write/
│   ├── cyberzen-layout/
│   ├── lark/
│   └── ...
│
├── devices/              ← 设备注册表，每个设备就是一个 skill 描述
│   ├── living-room-light/
│   │   └── SKILL.md      ← 品牌、协议、API 端点、支持的操作
│   ├── bedroom-ac/
│   │   └── SKILL.md
│   └── nas-synology/
│       └── SKILL.md
...
│       └── SKILL.md      ← 代理人，最小权限
│
└── inbox/                ← Agent 通信系统（邮箱 + 存档）
    ├── SKILL.md          ← 邮件系统说明：怎么发、怎么收、消息格式
    ├── {role}/           ← 每个角色有独立收件箱
    │   ├── incoming/     ← 收到的邮件
    │   ├── outgoing/     ← 发出的邮件
    │   └── archive/      ← 归档（已处理 + 过期），可搜索
    └── shared/           ← 跨角色的共享通知（如系统消息）
```

说明：

- 这棵树是逻辑结构，不一定要映射到文件系统的物理路径。后端可以用对象存储、数据库，只要对外暴露的 API 语义等价于文件操作（list / read / write / search）
...
信任等级决定 Agent 看到的文件树切面。低等级的 Agent 不是"没有权限访问 vault"，是 vault 这个目录在它的视野里根本不存在
...
- 内容：每天一个文件，记录当天所有 Agent 交互的摘要
- 写入方式：完全自动，Agent 交互结束后 Hub 自动生成
- 生命周期：默认保留 30 天，超过 30 天自动归档。如果其中有重要信息，应该已经被沉淀到 project 层
...
Hub 对外暴露的核心 API 语义就是文件操作：

| 操作 | 语义 | 说明 |
|------|------|------|
| `list(path)` | 列出目录内容 | 返回结果已按信任等级过滤 |
| `read(path)` | 读取文件内容 | vault 下的文件需要解密 |
| `write(path, content)` | 写入文件 | 只能写入有写权限的路径 |
| `search(query, scope)` | 全文搜索 | 搜索范围包括 memory + inbox，受信任等级限制 |
...
MCP 是首选协议（Claude 原生支持），HTTP API 做 fallback（所有平台都能用）。两者操作的是同一棵文件树，只是传输层不同
...
De 在管理后台：
  ...
  共享范围：memory/projects/industry-insights/（只读）
  有效期：30 天
...
Agent 给未来的自己写备忘。本质上就是把一条结构化信息写入 inbox/archive/，带上足够的元数据和标签，让未来的 Agent 实例可以检索到
...
对 Agent 来说，用户问"Q2 预算当时怎么调的"，它可以在 memory/projects/ 里找到项目上下文，也可以在 inbox/archive/ 里找到当时 Agent 间讨论预算的邮件往来。两种信息互补：项目上下文是结论性的摘要，邮件存档保留了决策过程

邮件的 TTL（过期时间）控制的是"是否还在收件箱里等待处理"，不是"是否删除"。过期的邮件自动从 incoming/ 移到 archive/，永远可检索
...
这一层的需求应该从 Phase 1-2 的实际使用中自然长出来。上面列的是方向，具体哪些先做取决于使用中暴露出来的真实需求
