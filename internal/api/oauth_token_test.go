...
func TestParseOAuthTokenRequestSupportsCapturedCursorAgentRequest(t *testing.T) {
	req := newRequestFromFixture(t, "testdata/oauth/cursor-agent/oauth_token_authorization_code.json")
	...
	if parsed.Code != "captured_cursor_agent_auth_code" {
		t.Fatalf("expected code from form body, got %q", parsed.Code)
	}
	if parsed.CodeVerifier != "CAPTURED_CURSOR_AGENT_CODE_VERIFIER" {
		t.Fatalf("expected code_verifier from form body, got %q", parsed.CodeVerifier)
	}
	if parsed.RedirectURI != "http://localhost:8787/callback" {
		t.Fatalf("expected redirect_uri from form body, got %q", parsed.RedirectURI)
	}
	if parsed.ClientID != "ahc_captured_cursor_agent_client" {
		t.Fatalf("expected client_id from form body, got %q", parsed.ClientID)
	}
	if parsed.ClientSecret != "ahs_captured_cursor_agent_secret" {
		t.Fatalf("expected client_secret from form body, got %q", parsed.ClientSecret)
	}
}
