...
当前仓库已经统一到单一 CLI：`agenthub`。

最简单的方式是一键安装：

```bash
git clone https://github.com/agi-bar/agenthub.git
cd agenthub
...
- 构建本地 `agenthub` binary
- 安装到一个可写的 bin 目录
- 如果该目录不在 `PATH`，自动写入你的 shell rc
...
如果这三条能正常运行，就说明本地 CLI 已经装好；后续 `agenthub` 会自动托管本地 Hub daemon，不需要你先手动起服务。

如果你想指定安装目录，也可以：

```bash
./tools/install-agenthub.sh --bin-dir ~/.local/bin --force
```

### 本地优先：安装后直接用 `agenthub`

Agent Hub 现在的主入口是单一命令 `agenthub`。默认心智是：

- 你安装一个本地工具
- 需要时它自动托管本地 Hub daemon
- 官方服务 `https://agenthub.agi.bar` 只是可选 remote profile
...
- `agenthub` 会自动管理本地 daemon 和本地 profile
- `agenthub mcp stdio` 保留为高级兼容模式，不是主路径
- 本地 daemon 现在默认使用 SQLite
...
```bash
# 后端
go run ./cmd/agenthub server --listen :8080
...
对于本地 CLI 平台，优先走 `agenthub connect ...`：

```bash
agenthub connect claude
agenthub connect codex
agenthub connect gemini
agenthub connect cursor
```

连接后，可以直接查看各平台的本地真实资产来源：

```bash
agenthub platform show claude
agenthub platform show codex
```

平台接入现在收口成两层：

- **MCP 能力面**：Agent Hub 的全量工具与文件树能力
- **本地入口层**：平台里安装的统一 `agenthub` 入口，用来把平台内对话路由到 MCP + 本地平台动作

当前首发体验：

- **Codex**
  - `agenthub connect codex` 会注册 Agent Hub MCP
  - 同时安装本地 skill 到 `~/.agents/skills/agenthub`
  - 在 Codex 中使用 `$agenthub export`、`$agenthub import`、`$agenthub list`、`$agenthub status`
- **Claude Code**
  - `agenthub connect claude` 会注册 Agent Hub MCP
...
2. 优先直接调用 Agent Hub MCP 工具 `import_skills_archive`
...
- **本地模式**：通过 `agenthub mcp stdio` 兼容接入
...
# Claude Code
claude mcp add -s user agenthub -- agenthub mcp stdio --token-env AGENTHUB_TOKEN

# Codex CLI
codex mcp add agenthub -- agenthub mcp stdio --token-env AGENTHUB_TOKEN
```
...
```bash
# 1. 如果要连接远端 Hub，先登录一次 remote profile
agenthub remote login official --url https://agenthub.agi.bar
...
- `AgentHub 入口技能`：系统级 umbrella skill，路径为 `/skills/agenthub/SKILL.md`
