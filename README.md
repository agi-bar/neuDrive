...
- **Web / Desktop Apps**：在 Claude Connectors、ChatGPT Apps 和 Cursor Desktop 中直接添加远程 MCP Server
- **CLI Apps（推荐）**：Claude Code、Codex CLI、Gemini CLI 通过远程 HTTP MCP + 浏览器 OAuth 授权接入，默认写入全局配置
...
- `任意 MCP 客户端` 这一路径仍覆盖能手工配置 MCP 的产品，例如 `Windsurf`、`GitHub Copilot` 等
- 除 Cursor 外，这些产品当前仍缺少 AgentHub 专用向导，但可以作为优先目标平台纳入后续 portability 和 setup 文档
...
- `Cursor` 已完成 Remote MCP + OAuth 连接验证，因此 `Hub 连接` 标记为 `✅`；后续仍需要补 portability 手册
- `GitHub Copilot`、`Windsurf` 已进入“通用 MCP 可接入”范围，但目前仍缺少 AgentHub 专用 setup/portability 文档，因此 `Hub 连接` 标记为 `📝`
