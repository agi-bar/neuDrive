...
`.skill` 文件统一注册。Agent 进来后读目录发现有什么可用，读 SKILL.md 知道怎么调用。支持批量导入 Claude 的 .skill 目录和记忆导出。
...
| **连接设置** | Claude / Codex 云端模式、本地模式、高级 MCP、GPT Actions、Token 管理（创建 / 改名 / 吊销） |
