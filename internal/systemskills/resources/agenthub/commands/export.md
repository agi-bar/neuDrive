...
Use this command when the user wants Agent Hub to prepare platform-oriented export materials.
...
- When the user asks to "put my Agent Hub into a repo", hand off to `agenthub git` instead of overloading `export`.

## Steps

1. Read `/skills/agenthub/SKILL.md`.
2. If the user explicitly wants a local Git repo for the Hub itself, read `/skills/agenthub/commands/git.md` and use `agenthub git init [--output DIR]` instead.
3. Read `/skills/agenthub/references/platforms/<platform>.md` if present.
...
15. If the probe fails in Claude Web, tell the user to open `Settings -> Capabilities -> Code execution and file creation -> Additional allowed domains`, add the Agent Hub host such as `agenthub.agi.bar`, and retry the direct upload.
...
20. All skill imports land under `/skills/<name>/...` in Agent Hub; a fallback upload flow should target the `/skills` root by default.
21. Write the result into Agent Hub through the chosen MCP or HTTP path, then report imported, archived, blocked items, and any active local Git mirror sync status explicitly.
