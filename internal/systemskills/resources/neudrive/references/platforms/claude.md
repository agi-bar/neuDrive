...
- Installed local skill: `neudrive`
...
- Use `export` to capture Claude-visible data into neuDrive.
...
- If that probe fails in Claude Web, tell the user to open `Settings -> Capabilities -> Code execution and file creation -> Additional allowed domains`, add the neuDrive host such as `neudrive.ai`, and then retry the direct upload.
...
- Use `import` to generate Claude-compatible materials from neuDrive.
