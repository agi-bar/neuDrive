...
export default function SkillsImportPage() {
  ...
  const requestError = useMemo(() => {
    if (request.error === 'missing_token') {
      return tx(
        '这个上传页面需要通过 neuDrive 生成的带 token 链接打开。',
        'This upload page must be opened through a tokenized link generated by neuDrive.',
      )
    }
    return ''
  }, [request.error, tx])
    ...
    <div className="skills-upload-page">
      <div className="skills-upload-card">
        ...
        <p className="login-desc">
          {tx(
            '把完整的 skills zip 上传到 neuDrive。页面会把内容导入 `/skills` 根目录，并反馈导入了哪些 skills。',
            'Upload a complete skills zip into neuDrive. The page imports it into the `/skills` root and reports which skills were imported.',
          )}
        </p>
